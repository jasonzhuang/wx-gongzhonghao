...
async function extractAuthor(page: Page): Promise<{ author: string; accountName: string }> {
...
async function extractPublishTime(page: Page): Promise<string> {
  return page.evaluate(() => {
...
async function extractContent(page: Page): Promise<{ html: string; text: string }> {
...
async function extractCoverImage(page: Page): Promise<string> {
  return page.evaluate(() => {
...
export async function extractArticle(page: Page, url: string): Promise<ArticleData> {
  const [title, { author, accountName }, publishTime, content, coverImage] =
    await Promise.all([
      extractTitle(page),
      extractAuthor(page),
